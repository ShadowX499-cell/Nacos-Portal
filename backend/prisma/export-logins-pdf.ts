/**
 * Generates: docs/student-logins.pdf
 * Lists all 300 seeded student accounts with UserID + Password.
 * Run: cd backend && npx ts-node prisma/export-logins-pdf.ts
 */
import 'dotenv/config';
import fs from 'fs';
import path from 'path';
import PDFDocument from 'pdfkit';
import { PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();

const GREEN  = '#166534';
const LGREEN = '#dcfce7';
const GRAY   = '#6b7280';
const BLACK  = '#111827';
const WHITE  = '#ffffff';
const ROW_ALT = '#f0fdf4';

const COLS = {
  no:       { x: 40,  w: 30  },
  userId:   { x: 70,  w: 140 },
  name:     { x: 210, w: 140 },
  program:  { x: 350, w: 55  },
  level:    { x: 405, w: 55  },
  password: { x: 460, w: 110 },
};
const ROW_H   = 16;
const PAGE_W  = 620;

function drawTableHeader(doc: PDFKit.PDFDocument, y: number) {
  // Header background
  doc.rect(40, y, PAGE_W - 40, ROW_H).fill(GREEN);
  doc.fillColor(WHITE).fontSize(7).font('Helvetica-Bold');
  const headers: [keyof typeof COLS, string][] = [
    ['no', '#'], ['userId', 'Student ID'], ['name', 'Full Name'],
    ['program', 'Program'], ['level', 'Level'], ['password', 'Password'],
  ];
  for (const [key, label] of headers) {
    doc.text(label, COLS[key].x + 2, y + 4, { width: COLS[key].w - 4, lineBreak: false });
  }
  return y + ROW_H;
}

function drawRow(
  doc: PDFKit.PDFDocument,
  y: number,
  i: number,
  row: { userId: string; name: string; program: string; level: string; password: string }
) {
  const bg = i % 2 === 0 ? WHITE : ROW_ALT;
  doc.rect(40, y, PAGE_W - 40, ROW_H).fill(bg);

  doc.fillColor(GRAY).fontSize(6.5).font('Helvetica');
  doc.text(String(i + 1), COLS.no.x + 2, y + 4, { width: COLS.no.w - 4, lineBreak: false });

  doc.fillColor(BLACK).font('Courier').fontSize(6.5);
  doc.text(row.userId, COLS.userId.x + 2, y + 4, { width: COLS.userId.w - 4, lineBreak: false });

  doc.font('Helvetica').fillColor(BLACK);
  doc.text(row.name, COLS.name.x + 2, y + 4, { width: COLS.name.w - 4, lineBreak: false });

  doc.fillColor(GRAY);
  doc.text(row.program, COLS.program.x + 2, y + 4, { width: COLS.program.w - 4, lineBreak: false });
  doc.text(row.level.replace('L', '') + 'L', COLS.level.x + 2, y + 4, { width: COLS.level.w - 4, lineBreak: false });

  doc.fillColor('#166534').font('Courier-Bold').fontSize(6.5);
  doc.text(row.password, COLS.password.x + 2, y + 4, { width: COLS.password.w - 4, lineBreak: false });

  return y + ROW_H;
}

async function main() {
  const students = await prisma.user.findMany({
    where: { role: 'student' },
    select: { userId: true, name: true, program: true, level: true },
    orderBy: [{ program: 'asc' }, { level: 'asc' }, { name: 'asc' }],
  });

  const rows = students.map((s) => ({
    userId:   s.userId,
    name:     s.name,
    program:  s.program,
    level:    s.level,
    password: `Nacos@${s.userId.split('/').pop()}`,
  }));

  const outDir = path.join(__dirname, '../../docs');
  if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
  const outPath = path.join(outDir, 'student-logins.pdf');

  const doc = new PDFDocument({ size: 'A4', layout: 'landscape', margin: 30 });
  const stream = fs.createWriteStream(outPath);
  doc.pipe(stream);

  let pageNum = 1;

  const printPage = () => {
    // Letterhead
    doc.rect(0, 0, doc.page.width, 50).fill(GREEN);
    doc.fillColor(WHITE).fontSize(14).font('Helvetica-Bold')
      .text('NACOS-AIFUE — Student Login Credentials', 40, 12, { lineBreak: false });
    doc.fontSize(8).font('Helvetica')
      .text(`Computer Science Department · ${rows.length} accounts · Generated ${new Date().toLocaleDateString('en-NG')}`,
        40, 30, { lineBreak: false });
    doc.fillColor(GRAY).fontSize(7)
      .text(`Page ${pageNum}`, doc.page.width - 80, 20, { lineBreak: false });
    pageNum++;
  };

  printPage();

  let y = 60;
  const BOTTOM_MARGIN = doc.page.height - 40;

  // Group header helper
  let currentGroup = '';

  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    const group = `${row.program} — ${row.level.replace('L', '')} Level`;

    // New page if close to bottom
    if (y + ROW_H > BOTTOM_MARGIN) {
      doc.addPage();
      printPage();
      y = 60;
      currentGroup = '';
      y = drawTableHeader(doc, y);
    }

    // Group header
    if (group !== currentGroup) {
      if (i === 0 || currentGroup !== '') {
        // Small spacing + group label
        y += 4;
        doc.rect(40, y, PAGE_W - 40, 14).fill(LGREEN);
        doc.fillColor(GREEN).fontSize(7.5).font('Helvetica-Bold')
          .text(group, 44, y + 3, { lineBreak: false });
        y += 14;
      }
      currentGroup = group;
      // Re-draw column header after group label
      y = drawTableHeader(doc, y);
    }

    y = drawRow(doc, y, i, row);
  }

  // Footer note
  y += 10;
  doc.rect(40, y, PAGE_W - 40, 22).fill('#fefce8');
  doc.fillColor('#92400e').fontSize(7).font('Helvetica-Bold')
    .text('⚠  CONFIDENTIAL — Password format: Nacos@<5-digit-ID-suffix>  |  Distribute securely  |  Students should change passwords on first login.',
      44, y + 7, { width: PAGE_W - 48, lineBreak: false });

  doc.end();

  await new Promise<void>((res, rej) => {
    stream.on('finish', res);
    stream.on('error', rej);
  });

  console.log(`✅ student-logins.pdf written → ${outPath}`);
  console.log(`   ${rows.length} students`);
}

main()
  .catch((e) => { console.error('❌', e); process.exit(1); })
  .finally(() => prisma.$disconnect());
